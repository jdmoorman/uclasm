...
def validate_alldiff_solns(exact_smp, marked,
    ...
    if recursive_validate(var_to_vars_list, vars_to_vals, vars_to_val_counts):
        ...
        for matched_pair in matched_pairs:
            if in_signal_only:
                ...
                print("marked_v", matched_pair)
        print(exact_smp)
        print(var_to_vars_list, vars_to_vals, vars_to_val_counts)
        return True
    return False

# TODO: switch to keyword arguments throughout
def validate_isomorphisms(exact_smp, unspec_cover, marked,
                          in_signal_only, node_to_marked_col_idx):
    ...
    if not exact_smp.candidates.any(axis=1).all():
        return False

    if len(unspec_cover) == 0:
        return validate_alldiff_solns(exact_smp, marked,
                                      in_signal_only, node_to_marked_col_idx)
    ...
    for cand_idx in unspec_cands:
        ...
        smp_copy = exact_smp.copy()
        smp_copy.candidates[unspec_idx, :] = uclasm.one_hot(cand_idx, world.n_nodes)

        # rerun filters after picking an assignment for the next unspec node
        uclasm.run_filters(smp_copy,
            filters=uclasm.cheap_filters,
            init_changed_cands=uclasm.one_hot(unspec_idx, tmplt.n_nodes))

        # if any node has no cands due to the current assignment, skip
        if not smp_copy.candidates.any(axis=1).all():
            continue

        if validate_isomorphisms(smp_copy,
            ...
            if in_signal_only:
                ...
                print("marked_in", unspec_idx, marked_col_idx)
            return True
    return False

def validation_filter(exact_smp, *, in_signal_only=False,
    ...
    while marked.any():
        ...
        unspec_subgraph = tmplt.node_subgraph(cand_counts > 1)
        unspec_cover = unspec_subgraph.node_cover()
        ...
        smp_copy.candidates[marked_tmplt_idx, :] = uclasm.one_hot(marked_cand_idx,
                                                              world.n_nodes)
        # rerun filters after picking an assignment for the next unspec node
        uclasm.run_filters(smp_copy,
            filters=uclasm.cheap_filters,
            init_changed_cands=uclasm.one_hot(marked_tmplt_idx, tmplt.n_nodes))

        # TODO: pass arguments as keywords to avoid bugs when changes are made
        if not validate_isomorphisms(smp_copy,
