...
for count_isomorphisms, timeout in [(False, 10000), (True, 10000)]:
    if False:
    ...
    if True:
        ...
        results = np.load("erdos_renyi_results_{}_trials_{}_layers{}{}_timeout_{}_vary_world_size.npy".format(n_trials, n_layers, "_count_iso" if count_isomorphisms else "", "_layerprobs" if layer_probs else "", timeout), allow_pickle=True)
        results_df = pd.DataFrame([x for x in results])
